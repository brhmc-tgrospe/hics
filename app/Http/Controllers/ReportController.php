...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!$user->hasRole(['Developer', 'Superadmin'])) {
            $wrappedQuery->where(function($q) use ($user) {
                // Allow General reports if generated by the user
                $q->where(function($subq) use ($user) {
                    $subq->where('report_type', 'General')
                ...
                } elseif ($user->hasRole(['Encoder', 'Secretary', 'Software'])) {
                    $q->orWhere(function($subq) use ($user) {
                        $subq->where('report_type', 'Area')
    ...
    private function canDeleteReport($user, $report, $areaDivisions)
    {
        ...
        if ($user->hasRole(['Encoder', 'Secretary', 'Software'])) {
