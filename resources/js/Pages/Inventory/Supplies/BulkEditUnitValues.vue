  ...
  <InventoryLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Bulk Edit Unit Values</h2>
          <p class="text-sm text-slate-500 font-medium mt-2">
            {{ supplies.length }} record{{ supplies.length !== 1 ? 's' : '' }} with missing unit values
          </p>
        </div>
      ...
      <!-- Info Banner -->
      <div v-if="supplies.length === 0" class="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
        <CheckCircleIcon class="w-12 h-12 text-green-500 mx-auto mb-3" />
        <h3 class="text-lg font-bold text-green-800">All Clear!</h3>
        <p class="text-sm text-green-600 mt-1">All supply records have unit values assigned.</p>
      </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-slate-50/80">
                ...
                <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest">UOM</th>
            ...
            <tbody class="divide-y divide-slate-100">
              <tr 
                ...
                :class="[
                  'transition-colors',
                  editedValues[supply.id] ? 'bg-blue-50/50' : 'hover:bg-slate-50/50'
                ]"
              >
                <td class="px-4 py-3 text-xs text-slate-400 font-mono">{{ index + 1 }}</td>
                <td class="px-4 py-3 text-sm font-bold text-slate-800">{{ supply.article }}</td>
                <td class="px-4 py-3 text-xs text-slate-600 max-w-[200px] truncate" :title="supply.description">{{ supply.description }}</td>
                <td class="px-4 py-3">
                  <span class="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase">{{ supply.category }}</span>
                </td>
                <td class="px-4 py-3 text-xs text-slate-600">{{ supply.unit_of_measure || '—' }}</td>
                <td class="px-4 py-3 text-xs text-slate-800 font-semibold text-right">{{ supply.on_hand_per_count }}</td>
                <td class="px-4 py-3 text-xs text-slate-600">{{ supply.division?.div_name || '—' }}</td>
                <td class="px-4 py-3">
                  <div class="relative">
                    ...
                    <input 
                      ...
                      step="0.01" 
                      min="0.01"
                      :value="editedValues[supply.id] ?? ''"
                      @input="handleInput(supply.id, $event.target.value)"
                      ...
                      :class="[
                        editedValues[supply.id] 
                          ? 'border-blue-400 focus:ring-blue-300 bg-blue-50/30' 
                          : 'border-slate-300 focus:ring-slate-400',
                        errors[supply.id] ? 'border-red-400 focus:ring-red-300 bg-red-50/30' : ''
                      ]"
                    />
                    <div v-if="errors[supply.id]" class="text-red-500 text-[10px] mt-0.5">{{ errors[supply.id] }}</div>
                  </div>
                </td>
                <td class="px-4 py-3 text-xs font-bold text-right" :class="computedTotal(supply) ? 'text-emerald-700' : 'text-slate-300'">
                  {{ computedTotal(supply) ? formatCurrency(computedTotal(supply)) : '—' }}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <!-- Fill All Helper -->
      <div v-if="supplies.length > 0" class="bg-white/40 backdrop-blur-sm border border-white/60 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div class="flex-1">
        ...
        <div class="text-xs text-slate-400">
          {{ editedCount }} of {{ supplies.length }} filled
        </div>
      </div>
    </div>
  </InventoryLayout>
...
const computedTotal = (supply) => {
  const unitValue = parseFloat(editedValues.value[supply.id]);
  if (isNaN(unitValue) || unitValue <= 0) return null;
  return unitValue * (supply.on_hand_per_count || 0);
};
...
const saveAll = () => {
  ...
  form.transform(() => ({ items })).put(route('supplies.bulk_update_values'), {
    preserveScroll: true,
  });
};
</script>
